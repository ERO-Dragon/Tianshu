...
| 层级 | 说明 | 存储建议 |
|---|---|---|
| Raw recent dialogue | 最近几轮原始对话 | Assistant 会话内存或短期缓存 |
| Session summary | 当前会话压缩摘要 | Assistant 状态 |
| Long-term memory | 跨会话稳定事实 | world/mod/agent 归属的持久文件 |
| Dynamic facts | 当前世界状态、玩家状态、临时事件 | 不持久化或短 TTL |

Assistant 不应把 dynamic facts 直接写成长期记忆。长期记忆必须经过筛选、压缩和去重。
...
Assistant 应根据 IA delivery 或 agent 注册信息确定：
...
如果这些字段直接写入 JSONL，服务端应忽略未知字段；但 Assistant 不应依赖服务端维护这些字段。
...
服务端会忽略 `*.tmp` 和 `*.lock`。Assistant 应避免在服务端读取过程中写半截文件。
...
```text
对话结束或达到压缩阈值
  -> Assistant 收集 raw dialogue + session facts
...
Assistant 有两种方式使用长期记忆：

### 7.1 Assistant 侧选择后注入 messages

Assistant 自己选择相关记忆，把它们整理为 system message。
...
Assistant 维护 `memories.jsonl`，common 启动服务端时传 `--memory-rag-path` 或未来的 `--rag-root-path`，请求时使用 `use_memory_rag`。

优点：

- 向量检索由服务端执行。
- 可返回 `rag_hits`，Assistant 可据此更新 hit_count/last_hit_time。
...
服务端返回的 `rag_hits.memory` 只表示本次实际注入 prompt 的记忆命中。Assistant 可以用它更新：
