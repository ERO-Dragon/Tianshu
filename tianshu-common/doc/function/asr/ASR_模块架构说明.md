...
- 核心功能
  ...
  - 将玩家的麦克风语音转换为最终文本。
  - 只有识别得到有效文本后，才发布 `INPUT_ASR_FINAL_TEXT` topic。

- 统一音频处理

  - 持续监听和按键说话采集的音频，都会先经过统一的音频处理链路。
  - 当前实际使用高通滤波；RNNoise 降噪仍是预留功能，尚未接入实际后端。

- 语音判断与自动分段

  - 持续监听会根据当前环境噪声动态调整说话判断门限。
  - 开始说话和结束说话采用不同门限，避免声音在临界状态下反复触发。
  - 过短的声音不会形成有效语音段，环境噪声突然变化后也会逐步恢复正常判断。
  - 玩家停止说话后，系统会等待一段安静时间再自动提交；当前语音持续越久，结束等待时间越短。
  - 说话状态变化时会发布 `INPUT_ASR_SPEECH_ACTIVITY` topic，但该状态不代表已经产生识别文本。

- 持续监听

  - 检测到玩家开始说话后，才开始记录一段语音。
  - 玩家可以等待自动分段，也可以按语音键提前提交当前有效语音。
  - 一段提交后，必须检测到下一次新的语音活动才会开始新的一段。
  - 没有新语音或有效文本时，重复提交不会产生识别 topic。

- 按键说话

  - 只采集玩家按住语音键期间的音频。
  - 松开语音键后提交识别，不使用自动语音分段。

- 设置与测试

  - 玩家可以选择麦克风和识别模型。
  - 提供独立试听功能，用于检查麦克风与模型是否正常，不介入正式语音链路。
...
5. **保留音频管线扩展位**
   - 高通、可用的 RNNoise 后端和轻量活动检测位于采集到识别之间；RNNoise 当前仍是预留接口，VAD 的动态分段策略已经作为 `TriggerMode.ALWAYS` 连续输入路径使用，PTT 不启用 VAD。
