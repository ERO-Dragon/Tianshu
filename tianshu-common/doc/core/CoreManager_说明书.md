...
- 整体作用
  ...
  - CoreManager 是天枢运行时的宿主，把 ASR、IR、IA、AX、LLM、TTS 等可选模块组织成一个完整系统。
  - 它统一推进模块的装配、准备、启动、停止和清理，并维护整个运行时的总体状态。

- 世界会话

  - 进入世界时创建新的运行会话，并重新装配本次会话需要的模块。
  - 退出世界时结束当前会话并清理会话资源；再次进入世界时重新建立一套有效模块实例。
  - 刷新请求会合并处理，避免同一时刻重复重建模块；客户端关闭后进入最终销毁状态。

- 状态与能力

  - 对外提供核心运行状态，以及各模块能力是否已经可用的状态。
  - 模块启动失败时保留失败结果，方便设置页、映迹和诊断功能了解真实运行情况。
