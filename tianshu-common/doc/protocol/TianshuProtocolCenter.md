...
- 整体作用
  ...
  - 协议中心是天枢各模块之间交换信息的统一通道。
  - ASR、IR、IA、AX、LLM、TTS 等模块通过它连接成完整的语音与对话链路。

- 信息传递方式

  - 能力用于向能够完成某项工作的模块发送请求或命令。
  - Topic 用于发布已经发生的事件，所有感兴趣的模块都可以订阅。
  - 请求产生的结果会沿原请求返回，流式任务可以连续返回内容并在最后给出完整终态。

- 消息秩序

  - 协议中心负责消息的路由、排队、顺序、完成、取消、过期和失败收口。
  - 不同消息可以选择等待、只保留最新状态、合并更新或在资源繁忙时拒绝，避免输入堆积和任务失控。
  - 每条请求都保留自己的身份和链路关系，因此取消或失败可以准确影响对应任务及其后续工作。
