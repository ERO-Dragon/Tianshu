...
public final class AXMemorySystem {
    ...
    private final AXCompressionTaskStore compressionTaskStore;
    ...
    public AXMemorySystem(AXStorageLayout layout, AXJsonStore jsonStore, AXMemoryWindowPolicy policy) {
        ...
        this.compressionTaskStore = new AXCompressionTaskStore(layout, jsonStore);
    ...
    public AXMemorySnapshot load(AXScope scope) {
        ...
        if (scope == null || !scope.writable()) {
            return new AXMemorySnapshot(persona, longTerm, List.of(), List.of());
        }
        ...
        return new AXMemorySnapshot(persona, longTerm, summary, blocks, shortTerm);
    }

    public void appendConversationTurn(AXScope scope, ConversationTurn turn) {
    ...
    public void appendMemoryUpdateCandidates(AXScope scope, List<MemoryUpdateCandidate> candidates) {
    ...
    public MemoryConsolidationPlan consolidatePendingMemory(AXScope scope, MemoryConsolidationPlanner planner) {
    ...
    private void applyConsolidationPlan(AXScope scope, MemoryConsolidationPlan plan) {
    ...
    private void appendWorldSummaryCandidates(AXScope scope, List<MemoryUpdateCandidate> candidates) {
    ...
    private List<MemoryUpdateCandidate> loadPendingWorldSummaryCandidates(AXScope scope) {
        if (scope == null || !scope.writable()) {
            return List.of();
        }
    ...
    private void writePendingWorldSummaryCandidates(AXScope scope, List<MemoryUpdateCandidate> candidates) {
        if (scope == null || !scope.writable()) {
            return;
        }
        List<JsonObject> objects = candidates == null ? List.of() : candidates.stream().filter(candidate -> !candidate.isEmpty()).map(this::candidateToJson).toList();
    ...
    public AXCompressionTask planNextCompressionTask(AXScope scope) {
        if (scope == null || !scope.writable()) {
        ...
        return compressionTaskStore.firstReady(scope, now)
                .orElseGet(() -> compressionTaskStore.firstActive(scope).isPresent()
                        ...
                        .orElseGet(() -> shortTermCompressionPlanner.plan(conversationWindowStore.loadRawTurns(scope))
                                ...
                                .orElse(AXCompressionTask.empty())));
    }

    public AXCompressionTask planShortTermCompressionTask(AXScope scope) {
        if (scope == null || !scope.writable()) {
            return AXCompressionTask.empty();
        }
    ...
    public ShortTermCompressionCandidate planShortTermCompression(AXScope scope) {
        if (scope == null || !scope.writable()) {
    ...
    public AXCompressionTask findCompressionTask(AXScope scope, String taskId) {
    ...
    public void failCompressionTask(AXScope scope, AXCompressionTask task, String errorCode, String errorMessage) {
        if (task != null && !task.isEmpty()) {
            compressionTaskStore.save(scope, task.fail(errorCode, errorMessage));
        }
    }

    public void markCompressionTaskSubmitted(AXScope scope, AXCompressionTask task) {
        if (task != null && !task.isEmpty()) {
            compressionTaskStore.save(scope, task.submitted());
        }
    }

    public void suspendCompressionTask(AXScope scope, AXCompressionTask task) {
        suspendCompressionTask(scope, task, "TASK_SUSPENDED", "Compression task is suspended");
    }

    public void suspendCompressionTask(AXScope scope, AXCompressionTask task, String code, String message) {
    ...
    public void acceptShortTermCompression(AXScope scope, ShortTermMemoryBlock block, int consumedTurnCount) {
        if (scope == null || !scope.writable() || block == null || block.isEmpty()) {
            return;
        }
        AXCompressionTask task = compressionTaskStore.firstActive(scope).orElse(AXCompressionTask.empty());
    ...
    public void acceptLongTermCompression(AXScope scope, AXCompressionTask task, String longTermMemory) {
    ...
    public void recordLongTermMemoryHits(AXScope scope, List<String> uids) {
        longTermMemoryRagStore.recordHits(scope, uids);
    }

    public void cleanupLongTermMemory(AXScope scope) {
        longTermMemoryRagStore.cleanup(scope);
    }

    public boolean hasMemoryRagEntries(AXScope scope) {
        return longTermMemoryRagStore.hasEntries(scope);
    }

    private void appendConversationSummary(AXScope scope, List<String> entries) {
    ...
    private List<String> loadSummary(AXScope scope) {
