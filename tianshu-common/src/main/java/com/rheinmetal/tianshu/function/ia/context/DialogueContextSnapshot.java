...
public record DialogueContextSnapshot(String playerId, String dimensionId, List<DialogueEntityRef> entityRefs, List<String> itemIds, Map<String, String> facts) {
    public DialogueContextSnapshot {
        ...
        itemIds = itemIds == null ? List.of() : List.copyOf(itemIds.stream().filter(value -> value != null && !value.isBlank()).map(String::trim).toList());
