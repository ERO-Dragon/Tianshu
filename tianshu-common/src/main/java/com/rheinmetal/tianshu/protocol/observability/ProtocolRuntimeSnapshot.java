...
public record ProtocolRuntimeSnapshot(List<BrokerSnapshot> brokers, List<ModuleDescriptor> modules, List<TopicDescriptor> topics, List<String> topicSubscriptionIds, List<String> directRouteIds, List<String> capabilityIds, List<VoiceTriggerRegistration> voiceTriggers, List<DeadLetterRecord> deadLetters, List<EnvelopeTransition> transitions, Map<String, Integer> stormRejects) {
    public static ProtocolRuntimeSnapshot from(ProtocolRuntime runtime) {
        return new ProtocolRuntimeSnapshot(runtime.brokers().snapshots(), runtime.modules().snapshot(), runtime.topics().snapshot(), runtime.topicSubscriptions().topicIds(), runtime.directRoutes().routeIds(), runtime.capabilities().capabilityIds(), runtime.voiceTriggers().registrations(), runtime.deadLetters().snapshot(64), runtime.lifecycle().allTransitions(), runtime.stormGuard().rejectionSnapshot());
    }
}
