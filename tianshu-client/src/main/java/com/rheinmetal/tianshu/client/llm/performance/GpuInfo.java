...
public final class GpuInfo {
    ...
    private static final ExecutorService DETECTION_EXECUTOR = new ThreadPoolExecutor(
            ...
            task -> {
                Thread thread = new Thread(task, "Tianshu-GPU-Detection");
    ...
    public static void requestRefresh(Runnable onComplete) {
        requestRefresh(System.currentTimeMillis(), onComplete);
    }

    static void close() {
        DETECTION_EXECUTOR.shutdownNow();
        CompletableFuture<Void> running = REFRESH_TASK.getAndSet(null);
        if (running != null) {
            running.cancel(true);
        }
    }

    public static GpuDevice selectedDevice(String selectedId) {
    ...
    private static void requestRefresh(long now, Runnable onComplete) {
        synchronized (REFRESH_TASK) {
            ...
            if (DETECTION_EXECUTOR.isShutdown()) {
                return;
            }
    ...
    public record GpuDevice(String id, String name, long vramTotalBytes, long vramUsedBytes, double utilization) {
        public GpuDevice {
            id = normalizeId(id);
