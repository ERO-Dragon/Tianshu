...
public final class TtsSettingsRegistrySource implements TianshuSettingsRegistrySource {
    ...
    private static final UiText TITLE = tts("title");
    private static final UiText DESCRIPTION = tts("description");

    private static UiText tts(String key, Object... args) {
        return UiText.key("tianshu.gui.tts." + key, args);
    }
    ...
    private final TtsSettingsAccess config;
    ...
    public TtsSettingsRegistrySource(TianshuCoreManager coreManager, TtsSettingsAccess config,
                                     ClientScheduler scheduler, ClientUiHost uiHost,
                                     ClientFilePicker filePicker, ClientTextProvider textProvider) {
    ...
    public void contribute(TianshuSettingsRegistry registry, ModuleSettingsContext context) {
        ...
        TtsSettingsDraft draft = new TtsSettingsDraft(config, coreManager, context, scheduler, uiHost, filePicker, textProvider);
    ...
    private static final class TtsSettingsDraft implements com.rheinmetal.tianshu.client.settings.session.ModuleSettingsSession {
        ...
        private TtsSettingsDraft(TtsSettingsAccess config, TianshuCoreManager coreManager, ModuleSettingsContext context,
                                 ClientScheduler scheduler, ClientUiHost uiHost,
                                 ClientFilePicker filePicker, ClientTextProvider textProvider) {
            this.config = config;
            this.coreManager = coreManager;
        ...
        private UiText selectedModelNameStatus() {
            TtsModelInfo info = resolveModel(selectedModelName.get());
            return info == null ? common("not_selected") : UiText.literal(info.name);
        }
        ...
        private UiText previewStatus() {
            return previewStatus;
        }

        private UiText failureStatus(TtsFailure failure) {
        ...
        private void buildDownloadItemActions(ModuleSettingsContext context, TtsModelInfo info, com.rheinmetal.tianshu.client.api.settings.ItemActionTemplate<TtsModelInfo> actions) {
        ...
        private void startDownload(ModuleSettingsContext context, TtsModelInfo info) {
            ...
            ttsModelService().downloadModel(info, githubProxyUrl.get(), new TtsModelService.DownloadProgressCallback() {
                ...
                public void onError(String message) {
                    runOnClient(() -> {
                        UiText downloadLabel = localizedKey(message, "status.download_failed");
        ...
        private UiText scoreFilterLabel(String value) {
            return ALL.equals(value) ? common("all") : tts("score.at_least", value);
        }

        private UiText voiceCloneOptionLabel(String value) {
            if (ALL.equals(value)) {
                return common("all");
            }
        ...
        private UiText voiceSampleOptionLabel(String value) {
            return NO_VOICE_SAMPLE.equals(value) ? common("not_selected") : UiText.literal(value);
        }

        private String languageLabel(TtsModelInfo info) {
            if (info == null || info.lang == null || info.lang.isEmpty()) {
                return textProvider.text(common("unknown"));
            }
        ...
        private UiText voiceLibraryPathStatus() {
            return UiText.literal(config.getVoiceLibraryPath().toString());
        }
